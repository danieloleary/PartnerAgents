---
title: Board Deck — Partner Program
section: Executive
category: executive
template_number: X.1
version: 2.0.0
last_updated: 2026-02-21
author: PartnerOS Team
tier:
skill_level: advanced
purpose: strategic
phase: enablement
time_required: 4-8 hours
difficulty: hard
prerequisites:
description: >
outcomes:
skills_gained:
keywords: ["story numbers matter", "template purpose", "use"]
---
> **Board presentations should tell a story. Numbers matter—but narrative matters more.** — Dan O'Leary

## How to Use This Template

**Purpose:**
This deck communicates partner program to the board.

**Steps:**

1. **Gather data** — Pull metrics (1 hour)
2. **Build narrative** — Tell the story (2 hours)
3. **Present** — Deliver to board (30 min)

---

# Board Deck Structure

## Slide Outline

| Slide | Content | Time |
|-------|---------|------|
| 1. Executive Summary | Key highlights | 2 min |
| 2. Strategy | Program vision | 3 min |
| 3. Performance | Results vs. plan | 5 min |
| 4. Metrics | KPIs | 3 min |
| 5. Roadmap | Future plans | 3 min |
| 6. Ask | Resource request | 2 min |

---

## Quick Use

- [ ] Gather metrics
- [ ] Build slides
- [ ] Present

---

## Related Templates

- [Strategy Plan](../strategy/05-strategy-plan/) — Program strategy
- [Success Metrics](../enablement/06-success-metrics/) — Track KPIs
- [Health Scorecard](../analysis/01-health-scorecard/) — Detailed assessment
