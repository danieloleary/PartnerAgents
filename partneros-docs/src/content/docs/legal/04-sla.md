---
title: Service Level Agreement
section: Legal
category: legal
template_number: L.4
version: 2.0.0
last_updated: 2026-02-22
author: PartnerOS Team
tier: Silver
skill_level: intermediate
purpose: operational
phase: enablement
time_required: 2-3 hours
difficulty: medium
prerequisites: [Support definitions, Legal review]
description: >
outcomes: [SLA terms, Support commitments, Performance standards]
skills_gained: [SLA design, Support management, Performance measurement]
keywords: ["upfront track", "template purpose", "use"]
---
> **Clear SLAs prevent arguments. Define them upfront, track them religiously.** — Dan O'Leary

## How to Use This Template

**Purpose:**
This agreement defines support levels and response times.

**Steps:**

1. **Define** — Set support levels (30 min)
2. **Document** — Create SLA terms (1 hour)
3. **Execute** — Agree with partner (1 hour)

---

# Service Level Agreement

## Support Levels

| Level | Response Time | Resolution Target |
|-------|---------------|-------------------|
| **P1 - Critical** | 1 hour | 4 hours |
| **P2 - High** | 4 hours | 24 hours |
| **P3 - Medium** | 24 hours | 72 hours |
| **P4 - Low** | 48 hours | 1 week |

---

## Quick Use

*Work with legal*

- [ ] Define support tiers
- [ ] Set response times
- [ ] Legal review
- [ ] Execute

---

## Related Templates

- [MSA](../legal/02-msa/) — Overall agreement
- [DPA](../legal/03-dpa/) — Data protection
- [Success Metrics](../enablement/06-success-metrics/) — Track SLA compliance
