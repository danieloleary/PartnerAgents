...
def format_partner_report(p: dict, generated_at: str) -> str:
    ...
    for name, data in playbooks.items():
        if data.get("completed"):
            ...
            pb_lines.append(f"  - ✅ **{name}** — completed {done_at}")
        else:
            step = data.get("current_step", 0)
            pb_lines.append(f"  - 🔄 **{name}** — step {step} in progress")
...
def generate_report(partners: list, generated_at: str) -> str:
    ...
    for p in partners:
        ...
        health_str = f"{health}/100" if health is not None else "—"
        completed = sum(
            1 for v in p.get("playbooks", {}).values() if v.get("completed")
        )
    ...
    partner_sections = "\n".join(
        format_partner_report(p, generated_at) for p in partners
    )

    return f"""# PartnerAgents Monthly Report
...
{partner_sections}
*Report generated by PartnerAgents — https://github.com/danieloleary/PartnerAgents*
"""


def main():
    parser = argparse.ArgumentParser(
        description="Generate monthly partner reports from PartnerAgents state"
    )
    parser.add_argument(
        "--partner", "-p", help="Single partner slug or name (default: all)"
    )
