...
def test_backend_sanitization_logic():
    ...
    user_message = "Hello <script>alert('xss')</script> world <img src=x onerror=alert(1)>"
...
def test_rate_limiting_logic():
    ...
    for i in range(20):
        assert check_rate_limit(ip, max_requests=20, window_seconds=60) is True

    # 21st request should be rate limited
    assert check_rate_limit(ip, max_requests=20, window_seconds=60) is False
...
def test_rate_limiter_memory_protection():
    ...
    assert len(rate_limit_store) <= 1002
...
def test_chat_input_validation():
    """Test /chat endpoint handles long inputs and empty inputs."""
    client = TestClient(app)

    # Empty message
    response = client.post("/chat", json={"message": "", "apiKey": "sk-or-test"})
    assert response.status_code == 200
    assert "Please enter a message" in response.json()["response"]

    # Very long message
    long_msg = "A" * 5001
    response = client.post("/chat", json={"message": long_msg, "apiKey": "sk-or-test"})
    assert response.status_code == 200
    assert "Message too long" in response.json()["response"]
